...
def expected_name_from_filename(filename):
...
def collect_probes(folder, suffix_filter=None, max_probes=None):
    ...
    return [
        (os.path.join(folder, f), expected_name_from_filename(f))
        for f in files
    ]
...
def fingerprint_clip(fp, audio_path, fan_value, delta_t_max, freq_bin_size):
    y, _ = fp.preprocess(audio_path)
    S_db = fp.generate_spectrogram(y)
    peaks = fp.find_peaks(S_db)
    return fp.generate_hashes(
        peaks,
        fan_value=fan_value,
...
def run_grid_search(conn, probes, label, fan_values, delta_t_max_values,
    ...
    grid = list(itertools.product(fan_values, delta_t_max_values,
                                  freq_bin_size_values, min_conf_values))

    print(f"\n{'='*80}")
    print(f"  Tuning on: {label}  ({len(probes)} probes, {len(grid)} combinations)")
    print(f"{'='*80}")
    print(f"  {'fan':>4} {'dt_max':>7} {'fbsz':>5} {'min_conf':>9}  "
    ...
    for fan_value, delta_t_max, freq_bin_size, min_conf in grid:
        ...
        print(f"  {fan_value:>4} {delta_t_max:>7} {freq_bin_size:>5} {min_conf:>9.3f}  "
...
def parse_args():
    parser = argparse.ArgumentParser(description="Tune fingerprinter parameters via grid search.")
    parser.add_argument(
        "--split", nargs="+", default=["clean", "light", "medium", "heavy"],
    ...
    parser.add_argument("--delta-t-max", nargs="+", type=int, default=list(DELTA_T_MAX_VALUES))
    parser.add_argument("--freq-bin-size", nargs="+", type=int, default=list(FREQ_BIN_SIZE_VALUES))
    parser.add_argument("--min-conf", nargs="+", type=float, default=list(MIN_CONF_VALUES))
...
def main():
    ...
    for split in args.split:
        if split == "clean":
            ...
            probes = collect_probes(NOISY_DIR, suffix_filter=f"_{split}",
